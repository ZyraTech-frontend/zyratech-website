export const collaborationHero = {
  bannerImage: '/images/Gemini_Generated_Image_7f3aff7f3aff7f3a.webp',
  bannerFallback: 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1600',
  title: 'Our Engagement Models Seamlessly Integrate With Your Operations',
  description:
    "At ZyraTech, we understand that every business has unique needs, which is why we've developed three flexible engagement models to cater to diverse client requirements. Each model is crafted to offer tailored solutions, ensuring that your organization benefits from enhanced efficiency and successful partnerships. Whether you are looking for dedicated teams, project-based engagements, or a blended approach, our models are designed to integrate seamlessly with your operations.",
  introImage: "/images/Dalene.webp",
  introFallback: 'https://images.unsplash.com/photo-1587614382346-acce7f3b8a31?w=1600'
};

export const collaborationModels = [
  {
    id: 'general-team-expansion',
    tab: 'General Team Expansion',
    title: 'General Team Expansion',
    bullets: [
      "Access specialized tech talent to boost your internal productivity and accelerate project timelines without the overhead of traditional hiring.",
      'This model offers unmatched flexibility and adaptability, empowering clients to scale their teams efficiently to meet evolving demands.',
      "Our team members become essential contributors, working side by side with the client's workforce to boost productivity and drive innovation.",
      'Dive into our offerings to see how we can partner with you in driving success and unlocking new opportunities.'
    ],
    image: "/images/image1.webp",
    imageFallback: 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1200'
  },
  {
    id: 'interdisciplinary-development-teams',
    tab: 'Interdisciplinary Development Teams',
    title: 'Interdisciplinary Development Teams',
    bullets: [
      "End-to-end project delivery managed by ZyraTech, utilizing Agile methodologies to ensure high-quality, custom-built solutions.",
      'Our full delivery team is dedicated to managing every aspect of the development process, ensuring seamless integration and execution.',
      "By bringing together diverse expertise and agile methodologies, this model ensures projects not only meet but exceed client expectations, driving innovation and success in dynamic business environments.",
      'Dive into our offerings to see how we can partner with you in driving success and unlocking new opportunities.'
    ],
    image: "/images/image2.webp",
    imageFallback: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1200'
  },
  {
    id: 'strategic-partnership-model',
    tab: 'Strategic Partnership Model',
    title: 'Strategic Partnership Model',
    bullets: [
      "A unique collaborative approach integrating ZyraTech's technical expertise with strategic media and industry partners like Royal Klast Group to maximize brand impact.",
      'This model is designed to harness the collective expertise of an interdisciplinary delivery team, ensuring a comprehensive and seamless development process.',
      'By leveraging diverse skills and perspectives, this model stands out as a prime choice for businesses seeking to optimize their operational effectiveness and achieve sustainable success.',
      'Dive into our offerings to see how we can partner with you in driving success and unlocking new opportunities.'
    ],
    image: "/images/image3.webp",
    imageFallback: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=1200'
  }
];

export const collaborationTestimonial = {
  quote:
    'As a small digital agency with an industry focus, we were looking for a capacity expansion for a new project. After the first exchange with ZyraTech, it quickly became clear that ZyraTech has exactly the competencies we need and that we have the chance to expand our team fundamentally and in the long term in cooperation with ZyraTech.',
  attribution: '– Partner feedback'
};

export const collaborationProcessSteps = [
  {
    step: 1,
    title: 'Discover',
    description: 'Understand needs, constraints and impact goals.'
  },
  {
    step: 2,
    title: 'Design',
    description: 'Co-design the solution and delivery plan.'
  },
  {
    step: 3,
    title: 'Deliver',
    description: 'Build, test and deploy the solution.'
  },
  {
    step: 4,
    title: 'Scale',
    description: 'Scale with partners and measure impact.'
  }
];
