...
const toolsData = [
  ...
  },
  {
    ...
    span: 'md:col-span-2',
    bgGradient: 'from-[#39366F]/10 to-[#2a2850]/10'
  },
  {
    category: 'DevOps',
    tools: [
    ...
    span: 'md:col-span-1',
    bgGradient: 'from-[#2a2850]/10 to-[#39366F]/10'
  },
  {
    category: 'API Tools',
    tools: [
    ...
    span: 'md:col-span-1',
    bgGradient: 'from-[#5B9BD5]/10 to-[#39366F]/10'
  },
  {
    ...
    span: 'md:col-span-2',
    bgGradient: 'from-[#39366F]/10 to-[#5B9BD5]/10'
  }
];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
      {toolsData.map((category, index) => (
        <div
          key={index}
          className={`${category.span} group relative bg-gradient-to-br ${category.bgGradient} backdrop-blur-sm border-2 border-gray-200/50 rounded-2xl sm:rounded-3xl p-5 sm:p-6 lg:p-7 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#39366F]/20 hover:border-[#39366F]/40 overflow-hidden min-h-[200px] sm:min-h-[220px] lg:min-h-0`}
        >
          {/* Animated gradient overlay on hover */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#39366F]/0 to-[#39366F]/0 group-hover:from-[#39366F]/8 group-hover:to-transparent transition-all duration-500 pointer-events-none"></div>
          ...
          <div className={`relative flex flex-col ${category.category === 'Cloud & Infrastructure' || category.category === 'Collaboration' || category.category === 'Testing & QA' || category.category === 'Design & UI' ? 'md:justify-center md:items-center md:h-full' : ''}`}>
            <h3 className="text-sm sm:text-base lg:text-lg font-bold uppercase tracking-wider text-gray-800 mb-4 sm:mb-5 group-hover:text-[#39366F] transition-colors duration-300 relative">
              {category.category}
              <span className="absolute -bottom-2 left-0 w-0 h-0.5 bg-[#39366F] group-hover:w-full transition-all duration-500"></span>
            </h3>
            <div className={`flex flex-col sm:flex-row sm:flex-wrap gap-2.5 sm:gap-3 lg:gap-4 ${category.category === 'Cloud & Infrastructure' || category.category === 'Collaboration' || category.category === 'Testing & QA' || category.category === 'Design & UI' ? 'md:justify-center' : ''}`}>
              {category.tools.map((tool, toolIndex) => (
                ...
                >
                  ...
                  />
                  <span className="text-xs sm:text-sm lg:text-base font-bold text-gray-800 group-hover/tool:text-[#39366F] transition-colors duration-300">
                    {tool.name}
                  </span>
                </div>
              ))}
